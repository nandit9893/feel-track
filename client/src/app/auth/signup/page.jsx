      ...
      <div className="w-full max-w-md rounded-3xl bg-white shadow-2xl relative overflow-hidden">
        ...
        <div className="h-full w-full flex flex-col gap-1 p-5 py-10 relative z-10">
          ...
          <form onSubmit={submitForm} className="flex flex-col gap-3 w-full">
            ...
            <div className="flex flex-col w-full">
              <p className="text-[16px] font-semibold text-gray-800">Email</p>
              <div className="flex justify-between items-center w-full gap-3">
            ...
            <div className="flex flex-col w-full">
              <p className="text-[16px] font-semibold text-gray-800">Password</p>
              <div className="flex justify-between items-center w-full gap-3">
                ...
                <input required name="password" onChange={inputChangeHandler} value={signUpFormData.password} type={passwordVisible ? "text" : "password"} className="text-black text-[16px] w-full font-light placeholder:text-gray-400 outline-none border-none bg-transparent" placeholder="Password"/>
                {
                  passwordVisible ? 
                  (
                    <EyeOff className="w-5 h-5 text-black cursor-pointer flex-shrink-0" onClick={() => setPasswordVisible(false)} />
                  ) 
                  : 
                  (
                    <Eye className="w-5 h-5 text-black cursor-pointer flex-shrink-0" onClick={() => setPasswordVisible(true)} />
                  )
                }
              </div>
              <hr className="w-full h-0.5 bg-gray-200" />
          ...
          <div className="flex flex-col w-full my-0.5">
            ...
            <div className="space-y-1">
              <div className="flex items-center gap-2">
                <CircleCheckBig className={`w-4 h-4 rounded-full ${passwordValidation.minLength ? "text-pink-500" : "text-gray-600"}`} />
                <p className={`${passwordValidation.minLength ? "text-pink-500" : "text-gray-600"} text-sm`}>Minimum 8 characters</p>
              </div>
              <div className="flex items-center gap-2">
                <CircleCheckBig className={`w-4 h-4 rounded-full ${passwordValidation.hasUpperCase ? "text-pink-500" : "text-gray-600"}`} />
                <p className={`${passwordValidation.hasUpperCase ? "text-pink-500" : "text-gray-600"} text-sm`}>At least one uppercase letter</p>
              </div>
              <div className="flex items-center gap-2">
                <CircleCheckBig className={`w-4 h-4 rounded-full ${passwordValidation.hasNumber ? "text-pink-500" : "text-gray-600"}`} />
                <p className={`${passwordValidation.hasNumber ? "text-pink-500" : "text-gray-600"} text-sm`}>At least one number</p>
              </div>
              <div className="flex items-center gap-2">
                <CircleCheckBig className={`w-4 h-4 rounded-full ${passwordValidation.hasSpecialChar ? "text-pink-500" : "text-gray-600"}`} />
                <p className={`${passwordValidation.hasSpecialChar ? "text-pink-500" : "text-gray-600"} text-sm`}>At least one special character</p>
              </div>
            </div>
          </div>
        </div>
