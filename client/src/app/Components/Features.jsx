    ...
    <div className="bg-black w-full h-full">
      <div className="max-w-7xl mx-auto flex flex-col gap-10 py-20">
        ...
        <div className="flex gap-5 items-center justify-center">
          {
            features.map((item) => (
              <div onMouseLeave={() => setCurrentItem(null)} onMouseEnter={() => setCurrentItem(item._id)} className="p-[2px] cursor-pointer rounded-xl bg-gradient-to-r from-purple-500 to-pink-500 flex-shrink-0" key={item._id}>
                <div className={`rounded-xl bg-slate-900 ${ currentItem === item._id ? "w-80" : "w-16"} h-[75vh] flex flex-col gap-5 p-4 transition-all duration-700 ease-in-out overflow-hidden`}>
                  ...
                  <div className={`transition-opacity duration-500 ${currentItem === item._id ? "opacity-100" : "opacity-0"} ${currentItem === item._id ? "block" : "hidden"}`}>
                    <Image src={item.image} alt={item.title} width={300} height={300} className="w-full h-56 rounded-2xl" priority unoptimized quality={100} />
                    <p className="text-lg font-semibold text-white my-5">{item.title}</p>
