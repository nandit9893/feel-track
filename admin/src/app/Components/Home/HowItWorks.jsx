...
const HowItWorks = ({ howHomeItWorksData }) => {
  ...
  useEffect(() => {
    if (howHomeItWorksData) {
      ...
      const initialSteps = {};
      howHomeItWorksData.steps?.forEach((step) => {
  ...
  return (
    <div id="homework" className="w-full bg-neutral-800 h-full p-10 flex flex-col gap-5">
      ...
      <div className="grid grid-cols-3 w-full gap-5 h-full">
        {
          howHomeItWorksData?.steps?.map((item) => (
            <div className="w-full border border-gray-400 rounded-md p-4 flex flex-col gap-5" key={item?._id}>
              {
                editItem === item?._id ?
                (
                  <div className="flex flex-col gap-2 w-full items-center">
                    <input type="text" className="p-2 text-gray-300 border border-gray-500 rounded-md outline-none w-full" value={editedSteps[item._id]?.heading} onChange={(e) => handleInputChange(item._id, 'heading', e.target.value)}/>
                    <textarea rows={12} className="p-2 text-gray-300 border border-gray-500 rounded-md outline-none w-full resize-none overflow-hidden" value={editedSteps[item._id]?.description} onChange={(e) => handleInputChange(item._id, 'description', e.target.value)}/>
                  </div>
                )
                :
                (
                  <div className="flex flex-col gap-2 w-full items-center">
                    <p className="text-gray-300 text-lg font-normal">{item?.heading}</p>
                    <p className="text-gray-400 text-base font-normal">{item?.description}</p>
                  </div>
                )
              }
              <div className="flex gap-4 w-full items-center">
                <button onClick={()=>setEditItem(item?._id)} className="text-base w-40 font-normal text-gray-300 py-2 px-6 rounded-lg bg-gradient-to-r from-neutral-900 to-neutral-800 border border-gray-600 hover:border-gray-500 hover:from-neutral-800 hover:to-neutral-700 transition-all duration-300 cursor-pointer shadow-lg hover:shadow-xl transform hover:-translate-y-0.5">EDIT</button>
